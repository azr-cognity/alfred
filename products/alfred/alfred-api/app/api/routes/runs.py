...
Cambios respecto a S11:
  - GET /runs/{id}: incluye cost_usd en la respuesta
  - GET /runs: incluye cost_usd en la lista
  - GET /runs/{id}/steps: nuevo endpoint — devuelve steps del run
...
GET  /api/runs/{id}         → detalle de un run (incluye cost_usd)
GET  /api/runs/{id}/steps   → steps del run ordenados por created_at
...
async def list_runs(
    ...
    async with AsyncSessionLocal() as session:
        ...
        if project_id:
            ...
            rows = await session.execute(
                text(base_select + "ORDER BY created_at DESC LIMIT :limit OFFSET :offset"),
                {"limit": limit, "offset": offset},
            )
    ...
    for r in runs:
        ...
        r["cost_usd"] = float(r["cost_usd"]) if r.get("cost_usd") is not None else None
...
async def get_run(run_id: str) -> dict:
    """Devuelve el detalle completo de un run, incluyendo cost_usd."""
    async with AsyncSessionLocal() as session:
        row = await session.execute(
            text("""
                SELECT id, prompt, status, current_agent, plan, result,
                       error, created_at, completed_at, project_id,
    ...
    data["cost_usd"] = float(data["cost_usd"]) if data.get("cost_usd") is not None else None

    return data


# --------------------------------------------------------------------------- #
# GET /api/runs/{run_id}/steps — steps del run
# --------------------------------------------------------------------------- #
@router.get("/runs/{run_id}/steps", tags=["Runs"])
async def get_run_steps(run_id: str) -> list:
    """Devuelve los steps de un run ordenados cronológicamente.

    Cada step incluye agent_name, status, output (task_id, summary,
    files_written, error) y cost_usd del agente.
    """
    async with AsyncSessionLocal() as session:
        rows = await session.execute(
            text("""
                SELECT id, run_id, agent_name, status, output,
    ...
    for s in steps:
        ...
        s["cost_usd"] = float(s["cost_usd"]) if s.get("cost_usd") is not None else None
